...
const getBackgroundColor = (backgroundColor) =>
  theme.colors[backgroundColor] || backgroundColor || theme.colors.interactive02;

const getColor = (color) => theme.colors[color] || color || theme.colors.inverse01;

const HomepageCallout = ({ leftText, rightText, backgroundColor: backgroundColorProp, color: colorProp, ...rest }) => {
  const backgroundColor = getBackgroundColor(backgroundColorProp);
  const color = getColor(colorProp);
  return (
    <Grid style={{ backgroundColor, color, width: '100%', maxWidth: '100%' }} className={grid} {...rest}>
      <Row className={row}>
  ...
  rightText: function renderRightText() {
    return (
      <p>
        ...
        <br />
        This is the guiding ethos behind IBM’s design philosophy and principles. This helps us distinguish every element
        and every experience Designed by IBM.
      </p>
    );
  },
};
