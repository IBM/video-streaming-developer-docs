...
import { footer, nav, listItem, logo } from 'gatsby-theme-carbon/src/components/Footer/Footer.module.scss';
import { grid, listItemVs } from './FooterSecondary.module.scss';
  ...
  return (
    <footer className={footer}>
      <Grid className={grid}>
        <Row>
          <Column colLg={4} colMd={4}>
            <ul className={nav}>
              {firstCol &&
                firstCol.map((link, i) => (
                  <li key={i} className={`${listItem} ${listItemVs}`}>
        ...
        <Row>
          <Column colLg={3}>
            <svg className={logo} width="81" height="32" xmlns="http://www.w3.org/2000/svg">
              <g fillRule="evenodd">
