...
To pass the response to the player your service has to redirect the page to the IBM Video Streaming return URL with passing the response in JSON format as a get parameter. You can test the API response at the [Hash Authentication Test](/viewer-authentication-api-hash-authentication-test) page.
