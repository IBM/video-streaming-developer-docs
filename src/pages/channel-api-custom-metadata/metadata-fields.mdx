...
```
POST https://api.ustream.tv/custom-metadata-fields.json
```
...
On success a response with HTTP status "200 OK" is returned with a set of key-value structures under a field element.
...
```
DELETE https://api.ustream.tv/custom-metadata-fields/{field_id}.json
```
