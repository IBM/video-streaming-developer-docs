...
On success a response with HTTP status "200 OK" is returned with an array under a chapters key. The elements of the array contain the following key-value pairs for each video chapter:
...
On success, a response with HTTP status "201 Created" is returned with the following key-value pairs under a `chapters` key.
...
On success a response with HTTP status "200 OK" is returned with the following key-value pairs under a `chapter` key.
