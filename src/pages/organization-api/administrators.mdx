...
This API calls allow to get all the Organizations accessible to the user represented by the token.
...
On success, a set of key-value pairs is returned under an `accounts` element with an HTTP Status code of 200.
...
```
GET https://api.video.ibm.com/organizations/{organizationId}/accounts/{accountId}/channels.json
```
...
```
GET /organizations/ZzzqXZ/accounts/12345678/channels.json HTTP/1.1
Host: api.video.ibm.com
Authorization: Bearer 3c2573673b782f6544405a22636f3d5d3b6f3950
```
...
On success, a set of key-value pairs is returned under a `channels` element with an HTTP Status code of 200.
...
| `lastBroadcastDate` | string  | Date of last broadcast on the channel |
| `channelStatus`     | string  | Possible values: `OFFLINE`, `LIVE`    |
| `managerNumber`     | number  | Number of managers on the channel     |
| `isOwn`             | boolean | The user is the owner of this channel |
| `ownerId`           | string  | Owners Account ID                     |
...
```json
{
  "channels": [
    {
      ...
      "url": "https://secure.video.ibm.com/channel/12345432",
      "thumbnailUrl": "https://ustvstaticcdn1-a.akamaihd.net/images/defaults/channel_66x66:4.png",
  ...
  "paging": {
      "previous": "https://api.video.ibm.com/organizations/ZzzqXZ/accounts/12345678/channels.json?page=1&pagesize=1",
      "next": "https://api.video.ibm.com/organizations/ZzzqXZ/accounts/12345678/channels.json?page=3&pagesize=1",
      "page_count": 3,
      "item_count": 3
  }
}
...
On success, a set of key-value pairs is returned under a `administrators` element with an HTTP Status code of 200.
...
| HTTP RESPONSE CODE      | ERROR CONDITIONS                                                                                                   |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------ |
| 400 Bad Request         | We couldn't add the new Administrator to your Organization with the provided parameters (eg.: user already added)  |
| 401 Unauthorized        | The provided access token is missing, revoked, expired or malformed                                                |
| 403 Forbidden           | The provided access token has no access to the user                                                                |
| 503 Service Unavailable | There is a temporary error on the server which makes it impossible to serve the request                            |
...
On success, a set of key-value pairs is returned with an HTTP Status code of 200.
...
If you update a user's access level she/he will lose any previously set permission under the Organization. 

Users modified to `limited` access can't access the Organization, you have to add Account/Channel level access right(s) to them. ([Update an administrator's Account level access](#update-an-administrator's-account-level-access), [Update an administrator’s channel level accesses](#update-an-administrator's-channel-level-accesses), [Set all Channel access levels](#set-all-channel-access-levels))
...
There must always be at least one Organization Administrator in an Organization, this call allows to check a user. 
...
This call allows to get every account under the Organization with the access level of the Administrator.
...
This call allows to get the Channels which the Administrator has specific access to. It also returns the access level. 

```
GET https://api.video.ibm.com/organizations/{organizationId}/administrators/{email}/accounts/{accountId}/channel-access.json
```
...
| PARAMETER  | TYPE   | IMPORTANCE | DESCRIPTION                                   |
| ---------- | ------ | ---------- | --------------------------------------------- |
| `channel`  | number | OPTIONAL   | Filter channels by name. Minimum 3 characters |
| `page`     | number | OPTIONAL   | Page number. The first page's index is 1      |
| `pagesize` | number | OPTIONAL   | Size of a page. 1 <= `pagesize` <= 50         |
...
```
GET /organizations/ZzzqXZ/administrators/amlsbEBleGFtcGxlLmNvbQ==/accounts/12345678/channel-access.json HTTP/1.1
Host: api.video.ibm.com
Authorization: Bearer 3c2573673b782f6544405a22636f3d5d3b6f3950
```
...
```
PUT https://api.video.ibm.com/organizations/{organizationId}/administrators/{email}/accounts/{accountId}/channel-access.json
```
...
| KEY      | TYPE   | IMPORTANCE | DESCRIPTION                                                                                                                                        |
| -------- | ------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `set`    | array  | REQUIRED   | Key-value pairs of Channel IDs and access levels. Access level: `channel.admin` or the ID of one of the Organization's [Custom Role](custom-roles) |
| `delete` | array  | REQUIRED   | List of Channel IDs which you want to revoke access from the Administrator                                                                         |
...
```
PUT /organizations/ZzzqXZ/administrators/amlsbEBleGFtcGxlLmNvbQ==/accounts/12345678/channel-access.json HTTP/1.1
Host: api.video.ibm.com
Authorization: Bearer 3c2573673b782f6544405a22636f3d5d3b6f3950
Content-Type: application/x-www-form-urlencoded

set[12348765]=channel.admin&set[12348764]=channel.admin&delete[]=123454&delete[]=123456
```
...
It is possible to set the access level to all Channels under an Account. With this call you can get the access level if it is previously set.

```
GET https://api.video.ibm.com/organizations/{organizationId}/administrators/{email}/accounts/{accountId}/all-channel-access.json
```
...
```
GET /organizations/ZzzqXZ/administrators/amlsbEBleGFtcGxlLmNvbQ==/accounts/12345678/all-channel-access.json HTTP/1.1
Host: api.video.ibm.com
Authorization: Bearer 3c2573673b782f6544405a22636f3d5d3b6f3950
```
...
```
PUT https://api.video.ibm.com/organizations/{organizationId}/administrators/{email}/accounts/{accountId}/all-channel-access.json
```
...
```
PUT /organizations/ZzzqXZ/administrators/amlsbEBleGFtcGxlLmNvbQ==/accounts/12345678/all-channel-access.json HTTP/1.1
Host: api.video.ibm.com
Authorization: Bearer 3c2573673b782f6544405a22636f3d5d3b6f3950
Content-Type: application/x-www-form-urlencoded
