...
#### Age lock (*deprecated*)
The requested content is only available for viewers above a certain age. This is represented by receiving an `onAgeLock()` callback.
After asking for the viewer's birthday, call `player.setBirthDate(date);` then restart the playback by `player.play();`.

*This feature is deprecated and it will be removed in version 2.1.0 of the SDK.*
