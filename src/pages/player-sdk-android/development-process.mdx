...
```java
private final PlayerFactory playerFactory = new PlayerFactory(USTREAM_PLAYER_SDK_KEY, context);
```
...
*   `onPasswordLock()` – Call `player.setPassword(password);` then restart the playback by `player.play();`

*   deprecated: ~~`onAgeLock()` – Call `player.setBirthDate(date);` then restart the playback by `player.play();`~~

*   `onHashLock()` – Call `player.setHash(hash);` then restart the playback by `player.play();`

Transient errors:

*   `onConnectionError()` – There is a problem with the internet connection,  
(most likely temporary) try again later by calling: `player.play();`

*   `onUnknownError()` – This indicates a problem that the Player SDK can't determine, 
(most likely temporary) try again by calling: `player.play();`
...
`new PlayerFactory(USTREAM_PLAYER_SDK_KEY, context);`
...
These callbacks are found in `com.ibm.video.android.player.api.ErrorListener`.
