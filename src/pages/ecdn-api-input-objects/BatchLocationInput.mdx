...
| - | - | - | - | - |
| name | [String](../ecdn-api-scalars/String) | NO | NO | Name of location.<br/>Must not be empty, must be unique and maximum allow length is 60 characters. | 
...
| externalRanges | [IPv4Range](../ecdn-api-scalars/IPv4Range) | NO | YES | External IP ranges of location. Omit if ranges are inherited.<br/>Cannot be filled if inheritExternalRanges is set to true. | 
| inheritExternalRanges | [Boolean](../ecdn-api-scalars/Boolean) | YES | NO | Whether external ranges are inherited from parent location.<br/>Defaults to false, must have a parent location if it's set to true.<br/>Cannot be true if externalRanges field is filled as well. | 
