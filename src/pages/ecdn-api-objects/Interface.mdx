...
| - | - | - | - | - |
| name | [String](../ecdn-api-scalars/String) | NO | NO | Name of network interface (ie. eth0, eth1, ens36, etc...). Currently it's always eth0 (multiple interfaces aren't supported yet). | 
...
| localIP | [IPv4](../ecdn-api-scalars/IPv4) | YES | NO | Local IP of interface. Null if network configuration mode is DYNAMIC and server never checked in before. | 
| netmask | [IPv4Mask](../ecdn-api-scalars/IPv4Mask) | YES | NO | Netmask of interface's local network. Null if network configuration mode is DYNAMIC and server never checked in before. | 
| gateway | [IPv4](../ecdn-api-scalars/IPv4) | YES | NO | Gateway of interface's local network. Null if network configuration mode is DYNAMIC and server never checked in before. | 
