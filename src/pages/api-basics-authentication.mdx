...
When access tokens expire or become invalid, applications can obtain a new access token using a refresh token without prompting users to enter their login credentials again. The refresh token is returned as a response of one of the previous token requests. Because refresh tokens are long-lived, developers should ensure that strict storage requirements are in place to keep them from being leaked.
...
In addition to the parameters above, the client has to provide its client secret (provided along with the client key) to authenticate itself if the client acts as a “web application”. Sending client secret can be omitted in case of “native application”.
