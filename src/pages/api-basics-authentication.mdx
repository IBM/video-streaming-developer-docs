...
- **Implicit:** Suitable for standalone, native clients (desktop / mobile).
...
2. After logging in, the browser is redirected to a special URL (defined by the client), passing the access token in the URL.
...
If the `response_type` is `token`, the following properties are returned:

| PROPERTY       | DESCRIPTION                                               |
| -------------- | --------------------------------------------------------- |
| `access_token` | Access token (40-character length, hex encoded sha1 hash) |
...
The following is an example with implicit flow, requesting for access tokens on the authorization page.
...
2a - The user enters his/her credentials and presses the Allow button. The browser is redirected to the following URL:
...
2b - The user cancels the authorization page or presses Deny after authentication. The browser is redirected to the following URL:

```
http://example.com/token?error=access_denied&state=XYZe38bcfdfbf46d659&redirect_uri=http://example.com/get_access_token
```

### Error responses

If there is an error, the token endpoint returns a HTTP status code indicating the error and a JSON object with the following format:
